...
class TestRuleFallbackPlan:
    ...
    def test_teacher_rating_has_no_retired_tool_fallback(self):
        orch, _plugin, reg = _make_orchestrator()
        plan = orch._rule_fallback_plan(
            _state(orch), self._cands(reg), "哪些老师评分高")
        assert plan is None

    def test_course_recommendation_has_no_retired_tool_fallback(self):
        orch, _plugin, reg = _make_orchestrator()
        plan = orch._rule_fallback_plan(
...
class TestLLMPlanFallback:
    ...
    def test_retired_llm_steps_are_rejected(self):
        orch, _plugin, reg = _make_orchestrator()
        ...
        assert plan is None or not plan.steps
