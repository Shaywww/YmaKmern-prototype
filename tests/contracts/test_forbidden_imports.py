...
SCAN_DIRS = ("packages/dududa-agent/src/dududa/core", "packages/dududa-agent/src/dududa/runtime")
BANNED_ROOTS = (
    "astrbot",
    "napcat",
    "onebot",
...
def test_core_dir_does_not_import_sibling_packages():
    ...
    violations = []
    for f in (ROOT / "packages" / "dududa-agent" / "src" / "dududa" / "core").rglob("*.py"):
        for lineno, line in enumerate(f.read_text(encoding="utf-8"), 1):
            m = IMPORT_RE.match(line)
            if not m:
                continue
            mod = m.group(1)
            if mod.startswith("dududa.") and not mod.startswith(
                ("dududa.core", "dududa.runtime")
            ):
                violations.append(
                    f"{f.relative_to(ROOT)}:{lineno}: {line.strip()}"
                )
