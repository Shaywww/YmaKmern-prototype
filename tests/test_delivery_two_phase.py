...
from packages.core.state import RuntimePhase, RunOutcome, RuntimeState
from packages.core.decision import SocialDecisionEngine
from packages.core.delivery import DeliveryReceipt, DeliveryStatus
from packages.core.memory import InMemoryRepository, MemoryScope, MemoryType
...
class TestTwoPhaseProtocol:
    ...
    async def test_duplicate_receipt_idempotent(self):
        ...
        records = memory.query(_bot_scope(), limit=20)
        bot = [r for r in records if "[嘟嘟哒]" in r.content]
        assert len(bot) == 1
