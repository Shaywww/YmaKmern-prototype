...
from packages.core.state import RunOutcome, SocialAction, RuntimeBudget
...
from packages.core.memory import InMemoryRepository, MemoryType, ScopeSelector
...
class TestDeliveryAckAndMemory:
    ...
    async def test_failed_receipt_skips_bot_memory(self):
        ...
        bots = repo.query_selector(ScopeSelector(memory_type=MemoryType.SHORT_TERM))
        assert epis
        assert not any("[嘟嘟哒]" in r.content for r in bots)
