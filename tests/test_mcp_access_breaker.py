...
from packages.mcp.access import (
...
from packages.core.state import RuntimeState, RuntimeBudget
...
class TestMCPProviderBreaker:
    ...
    class _OkService:
        ...
        async def search(self, **kw):
            from packages.mcp.base import ServiceResult
...
class TestOrchestratorScopeGating:
    def _orch(self, policy, monkeypatch, with_clock=True):
        import packages.runtime.orchestrator as orch_mod
    ...
    async def test_full_run_scope_gating(self, tmp_path, monkeypatch):
        ...
        def _make_orch(policy_path):
            reg = CapabilityRegistry()
            register_all_mcp_services(reg)
            reg.unregister("mcp.clock")  # 只剩 iCourse 服务，便于断言
