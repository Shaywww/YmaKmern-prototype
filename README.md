...
- 多模态接梗：被点名的图片会携带群上下文；未点名图片仅在闲聊语境、多人连续发表情包，或两人已围绕轻量问题形成短对话时进入视觉摘要与 DeepSeek 二次判断。普通照片与表情包保持不同类型，视觉结果以“画面/配文/情绪”结构化文本回填上下文
- 小群自然接话：至少 3 名成员围绕一个轻量问题形成短对话时，只把当前消息交给 DeepSeek 终审；仅 `casual_meme` 且置信度不低于 0.82 才回复，继续共用主动发言冷却和每日限额
