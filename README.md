...
- 天气城市守卫：城市参数只信消息文本中的地名（市/县/区/镇后缀或后跟「天气」），LLM 未提城市时统一默认合肥，不猜用户位置
...
- 可选“群聊问题补位”：默认关闭；群管理员开启后，仅在 4 分钟至少 15 条真人消息、3 人参与且最新消息为明确问题时回复；不随机 @，30 分钟冷却、每天最多 2 次
- 群消息前置门：无关消息在 UX 任务、进度、Trace 和模型调用前结束；图片拆条保持轻量暂存
- 群聊循环保护：静态发送者名单 + 重复/爆发/跨发送者回声熔断，真人精确 @ 可绕过动态熔断
- QQ 拆条 @ 窗口按平台/群/发送者/Bot 隔离、一次性消费，杜绝跨用户误配
