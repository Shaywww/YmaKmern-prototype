...
cmd_smoke() {
    ...
    check bash -n "$PROTO/scripts/ops.sh"
    check python3.12 -m py_compile \
        "$PROTO/packages/safeguards/limits.py" \
        "$PROTO/packages/application/dududa_prod.py" \
        "$PROTO/packages/application/dududa_core.py" \
        "$PROTO/packages/core/renderer.py"
    check python3.12 -c "import sys; sys.path.insert(0, '$PROTO'); sys.path.insert(0, '$PLUGIN'); import main"
    if [ "$fast" != "--fast" ]; then
        check python3.12 -m pytest "$PROTO/tests/test_p6_mcp_client.py" -q --tb=line
        check python3.12 -m pytest \
            "$PROTO/tests/test_runtime_limits.py" \
            "$PROTO/tests/test_renderer_delivery.py" -q --tb=line
    fi
